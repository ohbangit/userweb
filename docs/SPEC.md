...
interface Broadcast {
    ...
    endTime?: string | null // ISO 8601, 선택적 (종료 시간이 없을 수 있음)
    isLive: boolean
    isCollab: boolean
...
| Users    | 참여자   | 참여자 목록 표시 (주최자 배지 + 외부 링크 아이콘) |

#### 참여자 목록 정책

- **정렬**: 주최자(`isHost=true`) 우선, 동일 조건 내 한국어 이름 기준 오름차순 (`localeCompare('ko')`)
- **주최자 표기**: 주최자는 이름 우측에 `주최` 배지와 Crown 아이콘을 노출
...
- 일정 수정/추가 모달은 2열 구조로 구성한다.
    ...
    - 우측: 참석자 관리(리스트 우선 + 하단 추가, 이름 클릭 인라인 편집 + 자동완성 연결)
      시간 입력은 시작/종료 각각 `초기화` 버튼을 제공해 날짜 값을 즉시 비울 수 있다. 종료 시간은 선택적이며, 비어 있으면 `null`로 저장된다.
- 참석자는 이름만 저장할 수 있으며, 필요 시 관리 스트리머에 연결(`streamerId`)할 수 있다.
- 참석자 리스트에서 파트너 표시는 텍스트 라벨 대신 기존 파트너 아이콘(`mark.png`)으로 노출한다.
