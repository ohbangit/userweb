...
const ROLE_THEME: Record<OverwatchRole, { card: string; iconWrap: string; avatarRing: string }> = {
    TNK: {
...
function getMemberBroadcastUrl(member: { profileUrl: string | null; channelId: string | null }): string | null {
...
function MemberCardLink({ href, className, children }: { href: string | null; className: string; children: ReactNode }) {
    if (href === null) {
        return <div className={className}>{children}</div>
    }

    return (
        <a href={href} target="_blank" rel="noopener noreferrer" className={`${className} transition-colors hover:bg-[#07304f]`}>
            {children}
        </a>
    )
}
    ...
    return (
        <section className="w-full mt-10">
            ...
            {!collapsed && (
                <div className="mt-4">
                    {teams.length === 0 ? (
                        <p className="text-base text-[#6aadcc]/60">등록된 팀이 없습니다.</p>
                    ) : (
                        <div className="grid gap-3">
                            {teams.map((team) => (
                                <div key={team.id} className="w-full rounded-xl bg-[#062035] p-2.5">
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex flex-wrap justify-center gap-1.5">
                                            {team.members
                                                ...
                                                .sort((a, b) => PLAYER_SLOT_ORDER[a.slot] - PLAYER_SLOT_ORDER[b.slot])
                                                .map((member) => (
                                                    <MemberCardLink
                                                        key={member.id}
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <img
                                                                src={ROLE_IMG[member.slot as OverwatchRole]}
                                                                alt={member.slot}
                                                                className="h-4 w-4"
                                                            />
                                                        </div>
                                                        <div className="flex min-w-0 items-center gap-1.5">
                                                            ...
                                                            {member.isPartner && (
                                                                <img src={partnerMark} alt="파트너" className="h-3 w-3 shrink-0" />
                                                            )}
                                                        </div>
                                                    </MemberCardLink>
                                                ))}
                                        ...
                                        {team.members.some((member) => ['TNK', 'DPS', 'SPT'].includes(member.slot)) &&
                                            team.members.some((member) => ['HEAD_COACH', 'COACH'].includes(member.slot)) && (
                                                <div className="h-px w-full bg-gradient-to-r from-transparent via-[#1e3a5f]/70 to-transparent" />
                                            )}

                                        <div className="flex flex-wrap justify-center gap-1.5">
                                            {team.members
                                                .filter((member) => ['HEAD_COACH', 'COACH'].includes(member.slot))
                                                .sort((a, b) => STAFF_SLOT_ORDER[a.slot] - STAFF_SLOT_ORDER[b.slot])
                                                .map((member) => (
                                                    <MemberCardLink
                                                        key={member.id}
                                                        href={getMemberBroadcastUrl(member)}
                                                        className="relative flex min-w-0 items-center justify-between gap-1.5 rounded-lg bg-[#041524] px-2 py-1.5 sm:min-w-[170px]"
                                                    >
                                                        {member.isCaptain && (
                                                            <span className="absolute left-1 top-1 z-10 inline-flex items-center justify-center rounded-md bg-[#041524]/90 p-0.5 ring-1 ring-amber-400/50">
                                                                <Crown className="h-3 w-3 fill-amber-400 text-amber-400" />
                                                        ...
                                                        <span className="shrink-0 rounded-full bg-[#062035] px-1.5 py-0.5 text-xs font-semibold text-[#6aadcc]">
                                                            {SLOT_LABEL[member.slot] ?? member.slot}
                                                        </span>
                                                    </MemberCardLink>
                                                ))}
                                        </div>
