...
import { DEFAULT_PANEL_TITLE, getTournamentStatusBySchedule, isOverwatchGame, isRacingGame, toDraftParticipants, toTournamentDetailFromV2 } from '../utils/tournamentPromotion'
...
export default function TournamentPromotionPage() {
    ...
    const eventSchema = {
        ...
        url: seoUrl,
        organizer: hostName !== null ? { '@type': 'Organization', name: hostName, url: hostChannelUrl ?? undefined } : undefined,
    }

    if (isLoading) {
        return <div className="flex min-h-screen items-center justify-center bg-[#020d18]"><div className="h-10 w-10 animate-spin rounded-full border-2 border-[#0596e8] border-t-transparent" /></div>
    }
    ...
    const visiblePanels = promotionData.panels.filter((panel) => panel.enabled && !panel.hidden).sort((a, b) => a.order_index - b.order_index)
    const panelNavItems = visiblePanels.map((panel) => ({ id: `panel-${panel.id}`, label: panel.title_override ?? DEFAULT_PANEL_TITLE[panel.type] }))
    const draftPanel = promotionData.panels.find((panel) => panel.type === 'DRAFT')
    const draftPanelContent = draftPanel !== undefined && typeof draftPanel.content === 'object' && draftPanel.content !== null ? draftPanel.content : null
    const draftParticipants = toDraftParticipants(draftPanelContent as Record<string, unknown> | null)
    ...
    return (
        <div className="min-h-screen bg-[#020d18]">
            <Helmet>
                ...
                <meta name="keywords" content={isF1Tournament ? `${name}, F1, 레이스, 대회, 오뼱잋` : `${name}, 오버워치, 대회, 이스포츠, 오뼱잋`} />
                                    ...
                                    {panelNavItems.map((item) => (
                                        <li key={item.id}>
                                            <a
                                                href={`#${item.id}`}
                                                onClick={() => {
                                                    trackEvent('tournament_panel_nav_click', { slug: resolvedSlug, panel_label: item.label, panel_id: item.id })
                                                }}
